...
# * Model N deposition/percentage of natural/semi-natural habitat/land use-----------------------------------------------
# ** abundance model----------------------------------------------
AB_ND_PNH_land <- GLMER(modelData = model_data,responseVar = "logAbun",
                       fitFamily = "gaussian",
                       fixedStruct = "Predominant_land_use * NDRS * pnhRS",
                       randomStruct = "(1|SS)+(1|SSB)")

car::Anova(AB_ND_PNH_land$model)
summary(AB_ND_PNH_land$model)


#model output
tab_model(AB_ND_PNH_land, transform = NULL,
          show.icc = F, show.obs = F, show.ngroups = F)
# ** richness model----------------------------------------------
SR_ND_PNH_land <- GLMER(modelData = model_data,responseVar = "Species_richness",
                       fitFamily = "poisson",
                       fixedStruct = "Predominant_land_use * NDRS * pnhRS",
                       randomStruct = "(1|SS)+(1|SSB)+(1|SSBS)")

car::Anova(SR_ND_PNH_land$model)
summary(SR_ND_PNH_land$model)

# likelihood ratio test
SR_ND_PNH_land1<- glmer(Species_richness ~ Predominant_land_use*NDRS*pnhRS + (1|SS) +(1|SSB)+(1|SSBS),
                       data = model_data, family = "poisson",glmerControl(optimizer = "Nelder_Mead", optCtrl = list(maxfun = 100000)))

car::Anova(SR_ND_PNH_land1)
summary(SR_ND_PNH_land1)

SR_ND_PNH_land2<- glmer(Species_richness ~ NDRS+pnhRS+Predominant_land_use+NDRS:pnhRS+Predominant_land_use:pnhRS+NDRS:Predominant_land_use+ (1|SS) +(1|SSB)+(1|SSBS),
                       data = model_data, family = "poisson",glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

anova(SR_ND_PNH_land1,SR_ND_PNH_land2)
# * ★★★★★★★★★★★★★★★★★★---------------------------------------------------------
